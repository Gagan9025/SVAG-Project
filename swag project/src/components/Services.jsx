    ...
    <section id="services" className="py-16 md:py-20 relative">
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-4 animate-fade-in-down">
          <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-gold-400 font-heading">
            OUR SERVICES
          </h2>
        </div>
          ...
          {services.map((service, index) => (
            ...
            >
              <div className="flex items-start mb-4 md:mb-6">
                <div className="text-gold-400 mr-3 md:mr-4 mt-1 animate-bounce">{service.icon}</div>
                <div>
                  <span className="text-gold-400 text-xs md:text-sm font-semibold">{service.category}</span>
                  <h3 className="text-lg md:text-xl font-bold mt-1 text-ivory">{service.title}</h3>
                </div>
              </div>
              <p className="text-silver-400 mb-4 md:mb-6 text-sm md:text-base">{service.description}</p>
              <ul className="space-y-2 mb-4 md:mb-6">
                {service.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center animate-fade-in-right">
                    <span className="text-gold-400 mr-2 react-icon text-xs">✦</span>
                    <span className="text-silver-300 text-sm">{feature}</span>
                  </li>
                ))}
              </ul>
              <button 
                className="text-gold-400 font-semibold flex items-center group react-icon text-sm"
              >
                Learn more
        ...
        <div 
          className="mt-16 md:mt-20 text-center animate-fade-in-up"
          data-aos="fade-up"
          data-aos-delay="700"
        >
          {/* Aligned "Ready to Start?" heading with consistent styling */}
          <div className="mb-6 md:mb-8">
            <h3 className="text-2xl sm:text-3xl md:text-4xl font-extrabold mb-4 relative inline-block font-heading text-gold-400 animate-fade-in-down">
              READY TO START?
              <span className="absolute bottom-0 left-0 w-full h-1 bg-gradient-to-r from-transparent via-gold-400 to-transparent"></span>
            </h3>
            <div className="flex justify-center my-4 animate-pulse">
          ...
          <p className="text-base sm:text-lg md:text-xl text-silver-400 mb-6 md:mb-8 max-w-3xl mx-auto animate-fade-in">
            Let's discuss your project and create something amazing together.
          </p>
          <button 
            className="btn-gold inline-flex items-center group glow-pulse react-icon w-full sm:w-auto"
          >
            Start Your Project
            <FiArrowRight className="ml-2 react-icon" />
          </button>
